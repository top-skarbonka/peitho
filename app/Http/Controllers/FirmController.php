...
class FirmController extends Controller
{
    ...
    public function storeClientPoints(Request $request)
    {
        ...
        if (!$alreadyHadPointsRelation) {
            ...
            try {
                $smsResult = app(SmsApiSender::class)->send($phoneForSms, $message);
                ...
                return back()->with('success', "Dodano {$points} punktów i wysłano SMS z linkiem do rejestracji.");
            } catch (\Throwable $e) {
                DB::table('sms_logs')->insert([
                    'firm_id' => $firm->id,
                    'client_id' => $clientId,
                ...
                return back()->with('success', "Dodano {$points} punktów. SMS nie został wysłany.");
            }
        }
