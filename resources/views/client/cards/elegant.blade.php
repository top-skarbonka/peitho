...
.sticker{
    ...
    aspect-ratio:1;
    border-radius:50%;
    background:#e5e7eb; /* jasne, widoczne puste */
    opacity:1;
    transform:scale(1);
}
/* 🎯 EFEKT PIECZĄTKI */
.sticker.active{
    ...
    animation:stampDrop .55s cubic-bezier(.22,1.61,.36,1) forwards;
}
@keyframes stampDrop{
    0%{
...
<div class="glass-box">
    ...
    <div class="progress-bar">
        <div class="progress-fill" style="width: {{ min(100, ($current/$maxStamps)*100) }}%"></div>
    </div>

    @if($current < $maxStamps)
        Brakuje jeszcze <strong>{{ $maxStamps - $current }}</strong> do nagrody 🎁
    @else
        🎉 Nagroda gotowa!
    @endif
</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cardId = {{ $card->id }}; // ważne – per karta
    const storageKey = 'lastStamps_card_' + cardId;

    const stamps = document.querySelectorAll('.sticker');

    // 📦 poprzednia liczba pieczątek (z pamięci telefonu)
    const lastStamps = parseInt(localStorage.getItem(storageKey) || '0', 10);

    // 📳 wibracja – TYLKO jeśli doszła nowa pieczątka
    const shouldVibrate = current > lastStamps;
    ...
    stamps.forEach((el, i) => {
        if (i < current) {
            setTimeout(() => {
                ...
                // 👉 wibracja tylko na NOWEJ pieczątce
                if (shouldVibrate && i === current - 1) {
                    vibrateOnce();
                }
    ...
    // 💾 zapisz aktualny stan po animacji
